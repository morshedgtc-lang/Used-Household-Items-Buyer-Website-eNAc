...
async function main() {
  ...
  await prisma.city.createMany({
    data: cities.map((c) => ({ ...c, status: ContentStatus.PUBLISHED })),
  });
  ...
  const faqs = [
    ...
    },
    {
      ...
      answerAr: "نبوك والمناطق المجاورة. تواصل معنا للتأكد من التغطية.",
  ...
  await prisma.faq.createMany({
    data: faqs.map((f) => ({ ...f, status: ContentStatus.PUBLISHED })),
  });
